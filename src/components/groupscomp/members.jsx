...
const Members = () => {
  ...
  const handleShowMore = () => {
    setLoading(true);
    setTimeout(() => {
      setVisibleCount((prev) => Math.min(prev + 4, allMembersList.length));
      setLoading(false);
    }, 1500);
  };

  const visibleMembers = allMembersList.slice(0, visibleCount);
  ...
  return (
    <div className=" mx-auto  bg-white">
      ...
      {/* Show More Button */}
      {!loading && visibleCount < allMembersList.length && (
        <div className="flex">
          <button
