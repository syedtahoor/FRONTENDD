...
const PostImageStory = ({
  ...
  const createPostImageStory = async () => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/stories/postimage`,
        {
          post_id: postId,
          text_elements: textElements.length > 0 ? textElements : undefined,
          position_x: Number(imagePosition.x) || 0,
          position_y: Number(imagePosition.y) || 0,
          scale: Math.max(0.1, Math.min(5, Number(imageScale) || 1)),
          duration: 5,
        },
        {
          headers: {
